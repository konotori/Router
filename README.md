...
- Middleware hooks for guards, analytics, etc.  
...
- 🎯 Middleware for pre/post navigation/presentation hooks
...
Middleware provides powerful hooks for intercepting and responding to navigation events. Perfect for authentication guards, analytics, logging, and more.

#### Authentication Middleware

Block unauthorized users from accessing protected screens:
```swift
class AuthMiddleware: Middleware<AppRoute, AppSheet, AppCover> {
...
```swift
class AnalyticsMiddleware: Middleware<AppRoute, AppSheet, AppCover> {
...
// Add to router
router.addMiddleware(AnalyticsMiddleware(analytics: AnalyticsService.shared))
```
...
```swift
class LoggingMiddleware: Middleware<AppRoute, AppSheet, AppCover> {
    func shouldProcess(
        _ event: NavigationEvent<AppRoute>, 
        for router: BaseRouter<AppRoute, AppSheet, AppCover>
    ) -> Bool {
...
```swift
class FeatureFlagMiddleware: Middleware<AppRoute, AppSheet, AppCover> {
...
router.addMiddleware(FeatureFlagMiddleware(featureFlags: .shared))
```
...
router.addMiddleware(AnalyticsMiddleware(analytics: .shared))
```
...
### Middleware Tests
```swift
class MockAuthMiddleware: Middleware<AppRoute, AppSheet, AppCover> {
...
func testMiddlewareBlocksNavigation() {
    // Given
    let middleware = MockAuthMiddleware()
    middleware.shouldAllow = false
    ...
    // Then
    XCTAssertNil(router.currentRoute)
    XCTAssertEqual(middleware.processedEvents.count, 1)
}

func testMiddlewareAllowsNavigation() {
    // Given
    let middleware = MockAuthMiddleware()
    middleware.shouldAllow = true
    router.addMiddleware(middleware)
...
A: Use middleware! Create a middleware class that returns `false` in `shouldProcess()` to block navigation. Perfect for authentication checks.
...
A: Use middleware to intercept pop events, or restructure your navigation flow to use `.presentFullScreenCover()` for login/onboarding screens that shouldn't allow back navigation.
...
- **Middleware system**: allows adding analytics, logging, or navigation guards without needing to subclass the router.  
